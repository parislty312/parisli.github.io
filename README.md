## Tingyu Paris Li Website

This repository is a GitHub Pages personal website built from the Jekyll YAT template.
...
BUNDLER_VERSION=2.4.22 bundle exec jekyll serve --host 127.0.0.1 --port 4000
```
...
To stop the local preview server, press `Ctrl + C` in the terminal where Jekyll is running.

### Notes

- Dependencies are installed locally under `vendor/bundle`.
- Original template posts are kept in `template_posts/` for reference and excluded from the build.
- Deployment uses GitHub Actions via `.github/workflows/pages.yml`.
